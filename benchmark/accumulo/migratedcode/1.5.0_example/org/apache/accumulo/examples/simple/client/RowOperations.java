...
public class RowOperations {
	...
	public static void main(String[] args) throws AccumuloException, AccumuloSecurityException, MutationsRejectedException, TableExistsException, TableNotFoundException {
		...
		Scanner rowThree = RowOperations.getRow(opts, scanOpts, new Text("row3"));
		Scanner rowTwo = RowOperations.getRow(opts, scanOpts, new Text("row2"));
		Scanner rowOne = RowOperations.getRow(opts, scanOpts, new Text("row1"));
		...
		System.out.flush();
		rowTwo = RowOperations.getRow(opts, scanOpts, new Text("row2"));
		RowOperations.deleteRow(rowTwo);
		rowThree = RowOperations.getRow(opts, scanOpts, new Text("row3"));
		rowTwo = RowOperations.getRow(opts, scanOpts, new Text("row2"));
		rowOne = RowOperations.getRow(opts, scanOpts, new Text("row1"));
		RowOperations.log.info("This is row1 and row3");
		RowOperations.printRow(rowOne);
		RowOperations.printRow(rowTwo);
		RowOperations.printRow(rowThree);
		System.out.flush();
		RowOperations.deleteRow(opts, scanOpts, row1);
		rowThree = RowOperations.getRow(opts, scanOpts, new Text("row3"));
		rowTwo = RowOperations.getRow(opts, scanOpts, new Text("row2"));
		rowOne = RowOperations.getRow(opts, scanOpts, new Text("row1"));
	...
	private static void deleteRow(ClientOpts opts, ScannerOpts scanOpts, Text row) throws AccumuloException, AccumuloSecurityException, TableNotFoundException {
		RowOperations.deleteRow(RowOperations.getRow(opts, scanOpts, row));
	}
