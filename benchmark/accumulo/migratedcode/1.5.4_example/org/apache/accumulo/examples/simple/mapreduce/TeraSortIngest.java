...
public class TeraSortIngest extends Configured implements Tool {
	static class RangeInputFormat extends InputFormat<LongWritable, NullWritable> {
		...
		public List<InputSplit> getSplits(JobContext job) {
			long totalRows = TeraSortIngest.RangeInputFormat.getConfiguration(job).getLong(TeraSortIngest.NUMROWS, 0);
			int numSplits = TeraSortIngest.RangeInputFormat.getConfiguration(job).getInt(TeraSortIngest.NUMSPLITS, 1);
		...
		private static Configuration getConfiguration(JobContext job) {
			try {
				return ((Configuration) (TeraSortIngest.RangeInputFormat.GET_CONFIGURATION.invoke(job, new Object[0])));
			} catch (Exception e) {
