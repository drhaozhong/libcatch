...
public class FileDataQuery {
	...
	public FileDataQuery(String instanceName, String zooKeepers, String user, String password, String tableName, Authorizations auths) throws AccumuloException, AccumuloSecurityException, TableNotFoundException {
		ZooKeeperInstance instance = new ZooKeeperInstance(instanceName, zooKeepers);
		conn = instance.getConnector(user, password.getBytes());
		lastRefs = new ArrayList<Map.Entry<Key, Value>>();
	...
	public ChunkInputStream getData(String hash) throws IOException {
		...
		lastRefs.clear();
		PeekingIterator<Map.Entry<Key, Value>> pi = new PeekingIterator<Map.Entry<Key, Value>>(scanner.iterator());
		if (pi.hasNext()) {
