...
public class ARS {
	...
	public ARS.ReservationResult reserve(String what, String when, String who) throws Exception {
		...
		try (ConditionalWriter cwriter = conn.createConditionalWriter(rTable, new ConditionalWriterConfig());Scanner scanner = new IsolatedScanner(conn.createScanner(rTable, Authorizations.EMPTY))) {
			while (true) {
