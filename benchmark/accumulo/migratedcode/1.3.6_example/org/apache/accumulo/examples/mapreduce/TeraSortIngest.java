...
public class TeraSortIngest extends Configured implements Tool {
	static class RangeInputFormat extends InputFormat<LongWritable, NullWritable> {
		...
		public List<InputSplit> getSplits(JobContext job) {
			...
			int numSplits = job.getConfiguration().getInt(TeraSortIngest.ROWSPERSPLIT, 1);
	...
	public int run(String[] args) throws Exception {
		...
		conf.setLong(TeraSortIngest.NUMROWS, Long.parseLong(args[0]));
		...
		if ((args.length) > 10)
			conf.setInt(TeraSortIngest.ROWSPERSPLIT, Integer.parseInt(args[10]));
