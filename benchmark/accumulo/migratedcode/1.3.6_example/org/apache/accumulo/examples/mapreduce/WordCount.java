...
public class WordCount extends Configured implements Tool {
	...
	static {
		WordCount.usernameOpt = new Option("u", "username", true, "username");
		WordCount.passwordOpt = new Option("p", "password", true, "password");
		WordCount.opts = new Options();
		WordCount.opts.addOption(WordCount.usernameOpt);
		WordCount.opts.addOption(WordCount.passwordOpt);
	}
	...
	public int run(String[] unprocessed_args) throws Exception {
		...
		CommandLine cl = p.parse(WordCount.opts, unprocessed_args);
		String[] args = cl.getArgs();
		String username = cl.getOptionValue(WordCount.usernameOpt.getOpt(), "root");
		String password = cl.getOptionValue(WordCount.passwordOpt.getOpt(), "secret");
		if ((args.length) != 4) {
		...
		AccumuloOutputFormat.setOutputInfo(job, username, password.getBytes(), false, args[3]);
	...
	private int printUsage() {
		...
		hf.printHelp(WordCount.USAGE, WordCount.opts);
		return 0;
	}
