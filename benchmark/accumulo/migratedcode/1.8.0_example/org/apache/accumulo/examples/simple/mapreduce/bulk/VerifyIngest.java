		...
		for (int i = opts.startRow; i < (opts.numRows); i++) {
			if (si.hasNext()) {
				...
				if (!(entry.getKey().getRow().toString().equals(String.format("row_%010d", i)))) {
					VerifyIngest.log.error(((("unexpected row key " + (entry.getKey().getRow().toString())) + " expected ") + (String.format("row_%010d", i))));
					ok = false;
				}
				if (!(entry.getValue().toString().equals(String.format("value_%010d", i)))) {
					VerifyIngest.log.error(((("unexpected value " + (entry.getValue().toString())) + " expected ") + (String.format("value_%010d", i))));
					ok = false;
				}
			}else {
				VerifyIngest.log.error(("no more rows, expected " + (String.format("row_%010d", i))));
				ok = false;
				break;
			}
		}
