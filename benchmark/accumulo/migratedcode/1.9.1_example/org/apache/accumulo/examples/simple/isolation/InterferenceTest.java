...
public class InterferenceTest {
		...
		public void run() {
			...
			for (long i = 0; i < (iterations); i++) {
				...
				try {
					...
					InterferenceTest.log.error("Mutation was rejected.", e);
					System.exit((-1));
				}
			}
			try {
	...
	public static void main(String[] args) throws Exception {
		...
		Thread writer = new Thread(new InterferenceTest.Writer(conn.createBatchWriter(opts.getTableName(), bwOpts.getBatchWriterConfig()), opts.iterations));
		writer.start();
		InterferenceTest.Reader r;
		if (opts.isolated)
			r = new InterferenceTest.Reader(new IsolatedScanner(conn.createScanner(opts.getTableName(), opts.auths)));
		else
			r = new InterferenceTest.Reader(conn.createScanner(opts.getTableName(), opts.auths));
