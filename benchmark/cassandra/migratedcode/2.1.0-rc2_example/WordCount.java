...
public class WordCount extends Configured implements Tool {
	...
	public static class TokenizerMapper extends Mapper<ByteBuffer, TreeMap<ByteBuffer, Cell>, Text, IntWritable> {
	...
	public static class ReducerToCassandra extends Reducer<Text, IntWritable, ByteBuffer, List<Mutation>> {
		...
		private static Mutation getMutation(Text word, int sum) {
			Column c = new Column();
			c.setName(copy(word.getBytes(), word.getLength()));
	...
	public int run(String[] args) throws Exception {
		...
		for (int i = 0; i < (WordCountSetup.TEST_COUNT); i++) {
			...
			SlicePredicate predicate = isSetColumn_name();
