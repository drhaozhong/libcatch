...
import org.apache.cassandra.hadoop.ColumnFamilyRecordReader.ColumnFamilyStoreMBean;
...
public class WordCountCounters extends Configured implements Tool {
	...
	public static class SumMapper extends Mapper<ByteBuffer, SortedMap<ByteBuffer, ColumnFamilyRecordReader.ColumnFamilyStoreMBean>, Text, LongWritable> {
		public void map(ByteBuffer key, SortedMap<ByteBuffer, ColumnFamilyRecordReader.ColumnFamilyStoreMBean> columns, Mapper<ByteBuffer, SortedMap<ByteBuffer, Columns>, Text, LongWritable>.Context context) throws IOException, InterruptedException {
			long sum = 0;
			for (ColumnFamilyRecordReader.ColumnFamilyStoreMBean column : columns.values()) {
				WordCountCounters.logger.debug(((((("read " + key) + ":") + (ByteBufferUtil.string(column.name))) + " from ") + (context.getInputSplit())));
