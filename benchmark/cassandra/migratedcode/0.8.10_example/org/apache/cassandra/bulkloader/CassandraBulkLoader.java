...
public class CassandraBulkLoader {
	...
	public static class Reduce extends MapReduceBase implements Reducer<Text, Text, Text, Text> {
		...
		public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
			...
			for (InetAddress endpoint : StorageService.instance.getNaturalEndpoints(keyspace, ByteBufferUtil.Byte(key))) {
	...
	public static Message createMessage(String keyspace, byte[] key, String columnFamily, List<ColumnFamily> columnFamilies) {
		...
		Message message;
		Column column;
		baseColumnFamily = new ColumnFamily(ColumnFamilyType.Standard, IndexHelper.getComparator(keyspace, columnFamily), DatabaseDescriptor.getComparator(keyspace, columnFamily), CFMetaData.get(keyspace, columnFamily));
		for (ColumnFamily cf : columnFamilies) {
			bufOut.reset();
			ColumnFamily.serializer().serialize(cf, bufOut);
			byte[] data = new byte[bufOut.getLength()];
			System.arraycopy(bufOut.getData(), 0, data, 0, bufOut.getLength());
			column = new Column(ByteBuffer.wrap(data), ByteBuffer.wrap(data));
