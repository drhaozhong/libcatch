...
public class ClientOnlyExample {
	...
	private static void testReading() throws Exception {
		...
		for (int i = 0; i < 100; i++) {
			...
			List<Row> rows = StorageProxy.read(commands, ConsistencyLevel.ANY);
			assert (rows.size()) == 1;
			Row row = rows.get(0);
	...
	private static void setupKeyspace(Cassandra.Iface client) throws InvalidRequestException, SchemaDisagreementException, TException {
		...
		try {
			client.system_add_keyspace(new KsDef(ClientOnlyExample.KEYSPACE, ClientOnlyExample.KEYSPACE, cfDefList));
