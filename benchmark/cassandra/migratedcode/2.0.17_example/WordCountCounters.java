...
import static org.apache.cassandra.cli.CliClient.Function.values;
...
public class WordCountCounters extends Configured implements Tool {
	...
	public static class SumMapper extends Mapper<ByteBuffer, SortedMap<ByteBuffer, Column>, Text, LongWritable> {
		public void map(ByteBuffer key, SortedMap<ByteBuffer, Column> columns, Mapper<ByteBuffer, SortedMap<ByteBuffer, Column>, Text, LongWritable>.Context context) throws IOException, InterruptedException {
			long sum = 0;
			for (Column column : columns.values()) {
				WordCountCounters.logger.debug(((((("read " + key) + ":") + (values())) + " from ") + (context.getInputSplit())));
				sum += ByteBufferUtil.toLong(value());
			}
