...
public class SimpleAuthority implements IMutation {
	...
	public EnumSet<Permission> authorize(AuthenticatedUser user, List<Object> resource) {
		...
		if ((resource.size()) == 2) {
			keyspace = SimpleAuthority.KEYSPACES_WRITE_PROPERTY;
		...
		String accessFilename = System.getProperty(SimpleAuthority.ACCESS_FILENAME_PROPERTY);
		InputStream in = null;
		try {
			...
			if (keyspace == (SimpleAuthority.KEYSPACES_WRITE_PROPERTY)) {
				String kspAdmins = accessProperties.getProperty(SimpleAuthority.KEYSPACES_WRITE_PROPERTY);
	...
	public void validateConfiguration() throws ConfigurationException {
		String afilename = System.getProperty(SimpleAuthority.ACCESS_FILENAME_PROPERTY);
		if (afilename == null) {
			throw new ConfigurationException(String.format("When using %s, '%s' property must be defined.", this.getClass().getCanonicalName(), SimpleAuthority.ACCESS_FILENAME_PROPERTY));
		}
	}

	public void addAll(IMutation para0) {
		return;
	}

	public String toString(boolean para0) {
		return null;
	}
