...
public class WordCountCounters extends Configured implements Tool {
	...
	public static class SumMapper extends Mapper<ByteBuffer, SortedMap<ByteBuffer, Cell>, Text, LongWritable> {
		public void map(ByteBuffer key, SortedMap<ByteBuffer, Cell> columns, Mapper<ByteBuffer, SortedMap<ByteBuffer, Cell>, Text, LongWritable>.Context context) throws IOException, InterruptedException {
			long sum = 0;
			for (Cell cell : columns.values()) {
				WordCountCounters.logger.debug(((((("read " + key) + ":") + (cell.name())) + " from ") + (context.getInputSplit())));
	...
	public int run(String[] args) throws Exception {
		...
		SlicePredicate predicate = new SlicePredicate().setSlice_range(new SliceRange().setStart(ByteBufferUtil.EMPTY_BYTE_BUFFER).setFinish().setCount(100));
